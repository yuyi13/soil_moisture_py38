...
ds = gdal.Open('E:/Userdata/yuy/downloads/wetness_v3/ASCAT/ASCAT_0.05deg_Australia_20160101.tif')
a = ds.ReadAsArray()
...
# create NetCDF file
nco = netCDF4.Dataset('E:/Userdata/yuy/downloads/wetness_v3/ASCAT_combined.nc', 'w', clobber=True)
...
tmno = nco.createVariable('soil_moisture', 'd',  ('time', 'y_coordinate', 'x_coordinate'),
   zlib=True) # chunksizes=[chunk_time, chunk_lat, chunk_lon], fill_value=-9999
# tmno.units = 'm3/m3'
# tmno.scale_factor = ''
# tmno.add_offset = 0.00
tmno.long_name = 'soil moisture'
...
pat = re.compile('ASCAT_0.05deg_Australia_[0-9]{8}.tif')
itime = 0

#  step through data, writing time and data to NetCDF
for root, dirs, files in os.walk('E:/Userdata/yuy/downloads/wetness_v3/ASCAT/'):
    dirs.sort()
    files.sort()
    for f in files:
        if f.endswith('.tif'):
